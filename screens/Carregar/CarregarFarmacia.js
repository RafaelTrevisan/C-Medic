...
import Farmacia from "./Componentes/Farmacia";

//------------------------------------Classe---------------------------------------------------//
class CarregarFarmacia extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: []
...
//Função para Listar as Farmacias ja cadastradas
    componentDidMount() {
        var data = []
        db.transaction((tx) => {
            tx.executeSql(
                ...
                <Header androidStatusBarColor={'black'} style={{ backgroundColor: '#389B87' }}>
                    ...
                    <Right>
                        <Button iconRight transparent>
                            <Text style={styles.iconMenuCabecalho} onPress={() => this.props.navigation.navigate("Farmacia")}>Adicionar</Text>
                        </Button>
                    </Right>
                </Header>
                <FlatList
                    ...
                    renderItem={({ item }) => <Farmacia navigation = {this.props.navigation} data={item} />}
                />
            </Container>
        );
    }
...
export default CarregarFarmacia;