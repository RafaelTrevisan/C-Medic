...
import Cuidador from "./Componentes/Cuidador";

//------------------------------------Classe---------------------------------------------------//
class CarregarCuidador extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: []
...
//Função para Listar os cuidadores ja cadastrados
    componentDidMount() {
        var data = []
        db.transaction((tx) => {
            tx.executeSql(
                ...
                <Header androidStatusBarColor={'black'} style={{ backgroundColor: '#389B87' }}>
                    ...
                    <Right>
                        <Button iconRight transparent>
                            <Text style={styles.iconMenuCabecalho} onPress={() => this.props.navigation.navigate("Cuidador")}>Adicionar</Text>
                        </Button>
                    </Right>
                </Header>
                <FlatList
                    ...
                    renderItem={({ item }) => <Cuidador navigation = {this.props.navigation} data={item} />}
                />
            </Container>
        );
    }
...
export default CarregarCuidador;