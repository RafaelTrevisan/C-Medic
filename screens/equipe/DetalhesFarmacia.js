...
class FarmaciaScreen extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({ NumEnd: data.NumEnd })
    ...
    salvar = (nav, Codigo) => {
        ...
        db.transaction(function (tx) {
            tx.executeSql(
                'UPDATE Farmacia SET Nome = ?, Endereco = ?, Bairro = ?, NumEnd = ?, CEP = ?, Cidade = ?, Telefone = ?, Email = ? WHERE Codigo = ?',
                [Nome, Endereco, Bairro, NumEnd, CEP, Cidade, Telefone, Email, Codigo],
                (tx, results) => {
                ...
                <Header androidStatusBarColor={'black'} style={{ backgroundColor: '#389B87' }}>
                    <Left>
                        <Button icontLeft transparent>
                            ...
                            <Text style={styles.textHeader}>  Alterar Farmácia</Text>
                        </Button>
                    </Left>
                    <Right></Right>
                </Header>
